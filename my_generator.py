...
def preprocess_input(image, mask):
    ...
    mask_cat = keras.utils.to_categorical(np.array(mask / 127, dtype=np.int), num_classes=3)
    # mask = np.expand_dims(mask, axis=2)
    # new_mask_true=np.zeros((mask.shape[0],mask.shape[1],2), dtype=np.uint8)
    # inds = np.where(mask == 127)
    # new_mask_true[inds[0], inds[1],0] = 1.
    # inds2 = np.where(mask == 255)
    # new_mask_true[inds2[0], inds2[1],1] = 1.
    #
    # new_mask_inveres=np.ones((mask.shape[0],mask.shape[1], 1), dtype=np.uint8)
    # new_mask_inveres[inds[0], inds[1], 0] = 0.
    # new_mask_inveres[inds2[0], inds2[1], 0] = 0.
    #
    # new_mask = np.concatenate((new_mask_true,new_mask_inveres), axis=2)
...
# gen = train_generator(path2images, path2masks, batch_size=5)
